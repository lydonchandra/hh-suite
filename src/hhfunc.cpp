...
/////////////////////////////////////////////////////////////////////////////////////
void ReadQueryFile(Parameters &param, FILE *file, char &input_format,
                   char use_global_weights, HMM *queryHmm, Alignment *queryAlign, char infile[], float *pb,
                   const float S[20][20], const float Sim[20][20]) {
    std::unique_ptr<char[]> line_ptr(new char[LINELEN]);
    char *line = line_ptr.get();
    if (!fgetline(line, LINELEN, file)) {
        HH_LOG(ERROR) << "Error in " << __FILE__ << ":" << __LINE__ << ": " << __func__ << ":" << std::endl;
        HH_LOG(ERROR) << "\t" << infile << " is empty!\n";
        exit(4);
    }
    while (strscn(line) == NULL)
        fgetline(line, LINELEN, file); // skip lines that contain only white space
    ...
    if (!strncmp(line, "HMMER", 5)) {
        ...
        rewind(file);
        queryHmm->Read(file, param.maxcol, param.nseqdis, pb, path);
        input_format = 0;

        // HHM format
        if (input_format == 0) {
            HH_LOG(INFO) << "Extracting representative sequences from " << infile
                         << " to merge later with matched database sequences\n";
        }

        Alignment ali_tmp(param.maxseq, param.maxres);
        ali_tmp.GetSeqsFromHMM(queryHmm);
        ali_tmp.Compress(infile, param.cons, param.maxcol, param.M, param.Mgaps);
        *queryAlign = ali_tmp;
    }
        ...
        Alignment tempAlign(param.maxseq, param.maxres);

        // Read alignment from infile into matrix X[k][l] as ASCII (and supply first line as extra argument)
        tempAlign.Read(file, infile, param.mark, param.maxcol, param.nseqdis, line);

        // Convert ASCII to int (0-20),throw out all insert states, record their number in I[k][i]
        // and store marked sequences in name[k] and seq[k]
        tempAlign.Compress(infile, param.cons, param.maxcol, param.M, param.Mgaps);

        tempAlign.Shrink();

        // Sort out the nseqdis most dissimilar sequences for display in the output alignments
        tempAlign.FilterForDisplay(param.max_seqid, param.mark, S, param.coverage, param.qid, param.qsc, param.nseqdis);

        // Remove sequences with seq. identity larger than seqid percent (remove the shorter of two)
        tempAlign.N_filtered = tempAlign.Filter(param.max_seqid, S, param.coverage, param.qid, param.qsc, param.Ndiff);

        if (param.Neff >= 0.999)
            tempAlign.FilterNeff(use_global_weights, param.mark, param.cons, param.showcons, param.max_seqid,
                                 param.coverage,
                                 param.Neff, pb, S, Sim);

        // Calculate pos-specific weights, AA frequencies and transitions -> f[i][a], tr[i][a]
        tempAlign.FrequenciesAndTransitions(queryHmm, use_global_weights, param.mark, param.cons, param.showcons, pb,
                                            Sim);

        *queryAlign = tempAlign;
        input_format = 0;
    } else {
        HH_LOG(ERROR) << "Error in " << __FILE__ << ":" << __LINE__ << ": " << __func__ << ":" << std::endl;
    ...
    if (input_format == 0 && queryHmm->Neff_HMM > 11.0) {
        HH_LOG(WARNING) << "MSA " << queryHmm->name << " looks too diverse (Neff=" << queryHmm->Neff_HMM
                        << ">11). Better check it with an alignment viewer for non-homologous segments. Also consider building the MSA with hhblits using the - option to limit MSA diversity.\n";
    }
}
...
void PrepareTemplateHMM(Parameters &par, HMM *q, HMM *t, int format, float linear_tranistion_probs,
    ...
    t->IncludeNullModelInHMM(q, t, par.columnscore,
                             ...
                             pb); // Can go BEFORE the loop if not dependent on template
}
