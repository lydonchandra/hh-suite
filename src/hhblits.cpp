...
void HHblits::run(FILE* query_fh, char* query_path) {
    ...
    for (int round = 1; round <= par.num_rounds; round++) {
        ...
        *q_rescore = *query_hmm;
        ...
        if (par.premerge){
            premerge(previous_hits, premerged_hits, seqs_found, cluster_found, MINCOLS_REALIGN);
        }
        // Realign hits with MAC algorithm
        if (par.realign) {
...
void HHblits::premerge(Hash<Hit>* previous_hits, Hash<Hit>* premerged_hits,
    ...
    while (!hitlist.End() && prermergedHits<par.premerge)
    {
        ...
        if (prermergedHits>=imax(par.b,par.z) && hit_cur.Probab < par.p) break;
        if (prermergedHits>=imax(par.b,par.z) && hit_cur.Eval > par.E) continue;
        prermergedHits++;
