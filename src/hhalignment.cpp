...
//	std::cout << file << std::endl;
    ...
    while (firstline || (fgetline(line, LINELEN, inf))) {
        ...
        if (line[0] == '>') {
            if (k >= maxseq - 1) {
                HH_LOG(WARNING) << "Maximum number " << maxseq << " of sequences exceeded in file " << infile
                                << std::endl;
                break;
            }
            ...
            } else if (k == kss_dssp)  // lines with dssp secondary structure states (. - H E C S T G B)
            {
                while (h < LINELEN && line[h] > '\0' && l < maxcol - 1) {
                    if (ss2i(line[h]) >= 0 && ss2i(line[h]) <= 7) {
                        ...
                        l++;
                    } else HH_LOG(WARNING) << "Ignoring invalid symbol \'"
                                           << line[h] << "\' at pos. " << h
            ...
            } else if (k == kss_pred)  // lines with predicted secondary structure (. - H E C)
            {
                while (h < LINELEN && line[h] > '\0' && l < maxcol - 1) {
                    if (ss2i(line[h]) >= 0 && ss2i(line[h]) <= 3) {
                        cur_seq[l] = ss2ss(line[h]);
                        l++;
                    } else HH_LOG(WARNING) << "ignoring invalid symbol \'" << line[h]
                                           << "\' at pos. " << h << " in line "
                                           << linenr << " of " << infile << "\n";
                    h++;
                }
...
void Alignment::ReadCompressed(ffindex_entry_t *entry, char *data,
    ...
    while (index < data_size) {
        ...
        if (sequence_entry == NULL) {
            HH_LOG(ERROR) << "Could not fetch sequence entry: " << entry_index << " for alignment " << entry->name
                          << std::endl;
            exit(1);
        }
        ...
        if (sequence_data == NULL) {
            HH_LOG(ERROR) << "Could not fetch sequence data: " << entry_index << " for alignment " << entry->name
                          << std::endl;
            exit(1);
        }
        ...
        if (header_entry == NULL) {
            HH_LOG(ERROR) << "Could not fetch header entry: " << entry_index << " for alignment " << entry->name
                          << std::endl;
            exit(1);
        }
        ...
        char *header_data = ffindex_get_data_by_entry(ffindex_header_database_data,
                                                      header_entry);
        if (header_data == NULL) {
            HH_LOG(ERROR) << "Could not fetch header data: " << entry_index << " for alignment " << entry->name
                          << std::endl;
            exit(1);
        }
...
void Alignment::Compress(const char infile[], const char cons, const int maxcol, const int par_M, const int Mgaps) {
    ...
    switch (M) {
        ...
        default:
                ...
                if (keep[k])  //skip >ss_dssp, >ss_pred, >ss_conf, >aa_... sequences
                {
                    while ((c = seq[k][l++]))  // assign residue to c at same time
                    {
                        ...
                        else if (c != '.')  //match state = upper case character
                        {
                           ...
                           k == kss_pred)  // does alignment contain sequence of secondary structure states?
                {
                    while ((c = seq[k][l++]))  // assign residue to c at same time
                        if (c != '.' && !(c >= 'a' && c <= 'z'))
            ...
            if (L <= 0) {
                HH_LOG(ERROR) << "In " << __FILE__ << ":" << __LINE__ << ": " << __func__ << ":" << std::endl;
                HH_LOG(ERROR) << "\tAlignment in " << infile
                              << " contains no match states. Consider using -M first or -M <int> option" << std::endl;
                exit(1);
            }
...
int Alignment::Filter2(char keep[], int coverage, int qid, float qsc,
    ...
    char *in = new char[N_in +
                        1];  // in[k]=1: seq k has been accepted; in[k]=0: seq k has not yet been accepted at current seqid
    char *inkk = new char[N_in + 1];  // inkk[k]=1 iff in[ksort[k]]=1 else 0;
    int *Nmax = new int[L +
                        2];  // position-dependent maximum-sequence-identity threshold for filtering? (variable used in former version was idmax)
    int *idmaxwin = new int[L + 2];     // minimum value of idmax[i-WFIL,i+WFIL]
    int *seqid_prev = new int[N_in +
                              1];  // maximum-sequence-identity threshold used in previous round of filtering (with lower seqid)
    ...
    while (seqid <= seqid2) {
        ...
        for (kk = 0; kk < N_in; ++kk) {
            ...
            for (jj = 0; jj < kk; ++jj) {
                ...
                diff_suff = int(
                        diff_min_frac * imin(nres[k], cov_kj) + 0.999);  // nres[j]>nres[k] anyway because of sorting
                diff = 0;
...
void Alignment::FilterNeff(char use_global_weights, const char mark,
    ...
    do {
        if (y1 == y0) {
            HH_LOG(WARNING) << "Could not reduce diversity. Neff=" << y << " at filter threshold qsc=" << x
                            << std::endl;
            return;
        }
...
/////////////////////////////////////////////////////////////////////////////////////
void Alignment::FrequenciesAndTransitions(HMM *queryHmm, char use_global_weights,
    ...
    if (!queryHmm->dont_delete_seqs)  // don't delete sname and seq if flat copy to hit object has been made
    {
        for (k = 0; k < queryHmm->n_seqs; k++)
            delete[] queryHmm->sname[k];
        for (k = 0; k < queryHmm->n_seqs; k++)
            delete[] queryHmm->seq[k];
    } else  // Delete all not shown sequences (lost otherwise)
    {
        if (queryHmm->n_seqs > queryHmm->n_display) {
            for (k = queryHmm->n_display; k < queryHmm->n_seqs; k++)
                delete[] queryHmm->sname[k];
            for (k = queryHmm->n_display; k < queryHmm->n_seqs; k++)
                delete[] queryHmm->seq[k];
        }
    ...
    if (N_filtered > 1) {
        ...
        Amino_acid_frequencies_and_transitions_from_M_state(queryHmm, use_global_weights, in, pb);
        Transitions_from_I_state(queryHmm, in);  // use subalignments of seqs with insert in i
        Transitions_from_D_state(queryHmm,
                                 in);  // use subalignments of seqs with delete in i. Must be last of these three calls if par.wg==1!
    ...
    } else  // N_filtered==1
    {
        ...
        queryHmm->Neff_HMM = 1.0f;

        // for all positions i in alignment
        for (i = 0; i <= L + 1; ++i) {
            queryHmm->Neff_M[i] = 1.0f;
            queryHmm->Neff_I[i] = queryHmm->Neff_D[i] = 0.0f;
            for (a = 0; a < 20; ++a)
                queryHmm->f[i][a] = 0.0;
            if (X[k][i] < ANY)
                queryHmm->f[i][(unsigned int) X[k][i]] = 1.0;
            else
                for (a = 0; a < 20; ++a)
                    queryHmm->f[i][a] = pb[a];
            queryHmm->tr[i][M2M] = 0;
            queryHmm->tr[i][M2I] = -100000.0;
            queryHmm->tr[i][M2D] = -100000.0;
            queryHmm->tr[i][I2M] = -100000.0;
            queryHmm->tr[i][I2I] = -100000.0;
            queryHmm->tr[i][D2M] = -100000.0;
            queryHmm->tr[i][D2D] = -100000.0;
        }
        queryHmm->tr[0][I2M] = 0;
        queryHmm->tr[L][I2M] = 0;
        queryHmm->tr[0][D2M] = 0;
        queryHmm->Neff_M[0] = queryHmm->Neff_I[0] = queryHmm->Neff_D[0] = 99.999;  // Neff_av[0] is used for calculation of transition pseudocounts for the start state
    }
    ...
    if (Log::reporting_level() >= DEBUG) {
        ...
        for (i = 1; i <= imin(L, 100); ++i)
            HH_LOG(DEBUG) << i << "\t" << queryHmm->Neff_M[i] << "\t" << nseqs[i]
                          << std::endl;

        HH_LOG(DEBUG) << "Inserts:";
        HH_LOG(DEBUG) << "col\tNeff\tnseqs" << std::endl;
        for (i = 1; i <= imin(L, 100); ++i)
            HH_LOG(DEBUG) << i << "\t" << queryHmm->Neff_I[i] << "\t" << nseqs[i]
                          << std::endl;

        HH_LOG(DEBUG) << "Deletes:\n";
        HH_LOG(DEBUG) << "col\tNeff\tnseqs\n";
        for (i = 1; i <= imin(L, 100); ++i)
            HH_LOG(DEBUG) << i << "\t" << queryHmm->Neff_D[i] << "\t" << nseqs[i]
                          << std::endl;
    }
    ...
    queryHmm->L = L;
    queryHmm->N_in = N_in;
    queryHmm->N_filtered = N_filtered;
    for (i = 1; i <= L; ++i)
        queryHmm->l[i] = l[i];

    // Set names in HMM q
    if (strlen(queryHmm->name) == 0)
        strcpy(queryHmm->name, name);
    if (strlen(queryHmm->longname) == 0)
        strcpy(queryHmm->longname, longname);
    if (strlen(queryHmm->fam) == 0)
        strcpy(queryHmm->fam, fam);
    ScopID(queryHmm->cl,
           queryHmm->fold,
           queryHmm->sfam,
           queryHmm->fam);  // derive superfamily, fold and class code from family name
    strcpy(queryHmm->file, file);   // Store basename of alignment file name in q->file

    // Copy sequences to be displayed into HMM
    queryHmm->nss_dssp = queryHmm->nsa_dssp = queryHmm->nss_pred = queryHmm->nss_conf = queryHmm->nfirst = -1;
    int n = 0;
    if (kss_dssp >= 0)
        queryHmm->nss_dssp = n++;  // copy dssp sequence?
    if (ksa_dssp >= 0)
        queryHmm->nsa_dssp = n++;  // copy dssp sequence?
    if (kss_pred >= 0)
        queryHmm->nss_pred = n++;  // copy psipred sequence?
    if (kss_conf >= 0)
        queryHmm->nss_conf = n++;  // copy confidence value sequence?
    ...
    if (showcons || cons) {
        ...
        if (showcons) {
            ...
            queryHmm->ncons = n++;
            queryHmm->sname[queryHmm->ncons] = new char[10];
            if (!queryHmm->sname[queryHmm->ncons]) {
                MemoryError("array of names for displayed sequences", __FILE__,
                            __LINE__,
                            __func__);
            }
            strcpy(queryHmm->sname[queryHmm->ncons], "Consensus");
            queryHmm->seq[queryHmm->ncons] = new char[L + 2];
            if (!queryHmm->seq[queryHmm->ncons]) {
                MemoryError("array of names for displayed sequences", __FILE__,
                            __LINE__,
                            __func__);
            }
        ...
        if (cons) {
            ...
            queryHmm->nfirst = n++;
            kfirst = -1;
            queryHmm->sname[queryHmm->nfirst] = new char[strlen(name) + 11];
            if (!queryHmm->sname[queryHmm->nfirst]) {
                MemoryError("array of names for displayed sequences", __FILE__,
                            __LINE__,
                            __func__);
            }
            strcpy(queryHmm->sname[queryHmm->nfirst], name);
            strcat(queryHmm->sname[queryHmm->nfirst], "_consensus");
            queryHmm->seq[queryHmm->nfirst] = new char[L + 2];
            if (!queryHmm->seq[queryHmm->nfirst]) {
                MemoryError("array of names for displayed sequences", __FILE__,
                            __LINE__,
                            __func__);
            }
        ...
        for (i = 1; i <= L; ++i) {
            ...
            maxa = ANY;
            for (a = 0; a < 20; ++a)
                if (queryHmm->f[i][a] - pb[a] > maxw) {
                    maxw = queryHmm->f[i][a] - pb[a];
                    maxa = a;
                }
            ...
            if (showcons) {
                ...
                if (maxa < NAA) {
                    for (int b = 0; b < 20; b++) {
                        maxw += queryHmm->f[i][b] * Sim[maxa][b] * Sim[maxa][b];
                    }
                    maxw *= queryHmm->Neff_M[i] /
                            (queryHmm->Neff_HMM + 1);  // columns with many gaps don't get consensus symbol
                }
                if (maxw > 0.6)
                    queryHmm->seq[queryHmm->ncons][i] = uprchr(i2aa(maxa));
                else if (maxw > 0.4)
                    queryHmm->seq[queryHmm->ncons][i] = lwrchr(i2aa(maxa));
                else
                    queryHmm->seq[queryHmm->ncons][i] = 'x';
            }
            if (cons)
                queryHmm->seq[queryHmm->nfirst][i] = uprchr(i2aa(maxa));
        }
        if (showcons) {
            queryHmm->seq[queryHmm->ncons][0] = '-';
            queryHmm->seq[queryHmm->ncons][L + 1] = '\0';
        }
        if (cons) {
            queryHmm->seq[queryHmm->nfirst][0] = '-';
            queryHmm->seq[queryHmm->nfirst][L + 1] = '\0';
        }
    }
    ...
    for (k = 0; k < N_in; ++k) {
        int nn;
        if (display[k]) {
            if (n >= queryHmm->maxseqdis) { //MAXSEQDIS) {
                if (mark)
                    HH_LOG(WARNING) << "Maximum number " << queryHmm->maxseqdis
                                    << " of sequences for display of alignment exceeded" << std::endl;
                break;
            }
            if (k == kss_dssp)
                nn = queryHmm->nss_dssp;  // copy dssp sequence to nss_dssp
            else if (k == ksa_dssp)
                nn = queryHmm->nsa_dssp;
            else if (k == kss_pred)
                nn = queryHmm->nss_pred;
            else if (k == kss_conf)
                nn = queryHmm->nss_conf;
            else if (k == kfirst)
                nn = queryHmm->nfirst = n++;
            else
                nn = n++;
//        strcut(sname[k],"  "); // delete rest of name line beginning with two spaces "  " // Why this?? Problem for pdb seqs without chain
            queryHmm->sname[nn] = new char[strlen(sname[k]) + 1];
            if (!queryHmm->sname[nn]) {
                MemoryError("array of names for displayed sequences", __FILE__,
                            __LINE__,
                            __func__);
            }
            strcpy(queryHmm->sname[nn], sname[k]);
            queryHmm->seq[nn] = new char[strlen(seq[k]) + 1];
            if (!queryHmm->seq[nn]) {
                MemoryError("array of names for displayed sequences", __FILE__,
                            __LINE__,
                            __func__);
            }
            strcpy(queryHmm->seq[nn], seq[k]);
        }
    }
    queryHmm->n_display = n;  // how many sequences to be displayed in alignments?
    queryHmm->n_seqs = n;

    // Copy secondary structure information into HMM
    if (kss_dssp >= 0)
        for (i = 1; i <= L; ++i)
            queryHmm->ss_dssp[i] = X[kss_dssp][i];
    if (ksa_dssp >= 0)
        for (i = 1; i <= L; ++i)
            queryHmm->sa_dssp[i] = X[ksa_dssp][i];
    if (kss_pred >= 0) {
        for (i = 1; i <= L; ++i)
            queryHmm->ss_pred[i] = X[kss_pred][i];
        if (kss_conf >= 0)
            for (i = 1; i <= L; ++i)
                queryHmm->ss_conf[i] = X[kss_conf][i];
        else
            for (i = 1; i <= L; ++i)
                queryHmm->ss_conf[i] = 5;
    }

    queryHmm->lamda = 0.0;
    queryHmm->mu = 0.0;

    queryHmm->trans_lin = 0;  // transition probs in log space
    queryHmm->has_pseudocounts = false;
    queryHmm->dont_delete_seqs = false;
    queryHmm->divided_by_local_bg_freqs = false;
    ...
    if (Log::reporting_level() >= DEBUG) {
        ...
        for (i = 1; i <= L; ++i) {
            HH_LOG(DEBUG) << i << " ";
            for (a = 0; a < 20; ++a)
                HH_LOG(DEBUG) << 100 * queryHmm->f[i][a] << " ";
        ...
        HH_LOG(DEBUG)
            ...
            << std::endl;
        for (i = 0; i <= L; ++i) {
            HH_LOG(DEBUG) << i << "  " << fpow2(queryHmm->tr[i][M2M]) << " "
                          << fpow2(queryHmm->tr[i][M2I]) << " "
                          << fpow2(queryHmm->tr[i][M2D]) << " ";
            HH_LOG(DEBUG) << fpow2(queryHmm->tr[i][I2M]) << " "
                          << fpow2(queryHmm->tr[i][I2I]) << " ";
            HH_LOG(DEBUG) << fpow2(queryHmm->tr[i][D2M]) << " "
                          << fpow2(queryHmm->tr[i][D2D]) << "  ";
            HH_LOG(DEBUG) << queryHmm->Neff_M[i] << " " << queryHmm->Neff_I[i] << " "
                          << queryHmm->Neff_D[i] << std::endl;
        }
    }
}
...
// Pos-specific weights are calculated like in "GetPositionSpecificWeights()"
/////////////////////////////////////////////////////////////////////////////////////
void Alignment::Amino_acid_frequencies_and_transitions_from_M_state(HMM *q, char use_global_weights, char *in,
                                                                    const float *pb) {
    ...
    if (use_global_weights == 1)  // If global weights
        ...
        unsigned int NAA_VECSIZE = ((NAA + 3 + VECSIZE_INT - 1) / VECSIZE_INT) *
                                   VECSIZE_INT; // round NAA+3 up to next multiple of VECSIZE_INT
        n = new int *[L + 2];
        for (j = 1; j <= L; ++j)
    ...
    for (i = 1; i <= L; ++i)  // Calculate wi[k] at position i as well as Neff[i]
    {
        ...
        if (use_global_weights == 0) {
            ...
            for (k = 0; k < N_in; ++k) {
                ...
                if (X[k][i - 1] >= ANY &&
                    X[k][i] < ANY) {  // ... if sequence k was NOT included in i-1 and has to be included for column i
                    change = 1;
                    nseqi++;
                    for (int j = 1; j <= L; ++j)
                        n[j][(int) X[k][j]]++;
                } else if (X[k][i - 1] < ANY && X[k][i] >=
                                                ANY) {  // ... if sequence k WAS included in i-1 and has to be thrown out for column i
                    change = 1;
                    nseqi--;
                    for (int j = 1; j <= L; ++j)
                        n[j][(int) X[k][j]]--;
...
//       for (k=0; k<N_in; ++k) if (in[k]) w[k][i]=wi[k];
    ...
    if (use_global_weights == 1) {
        ...
        float scale = flog2((Nlim - q->Neff_HMM) /
                            (Nlim - 1.0));  // for calculating Neff for those seqs with inserts at specific pos
        for (i = 1; i <= L; ++i) {
...
void Alignment::Transitions_from_D_state(HMM *q, char *in) {
    ...
    if (1) {
        ...
        Nlim = fmax(10.0, q->Neff_HMM + 1.0);    // limiting Neff
        scale = flog2(
                (Nlim - q->Neff_HMM) / (Nlim - 1.0));  // for calculating Neff for those seqs with dels at specific pos
    }
        ...
        if (0) {
            ...
            for (k = 0; k < N_in; ++k) {
                if (!in[k])
                    continue;
                if (X[k][i - 1] != GAP &&
                    X[k][i] == GAP) {  // ... if sequence k was NOT included in i-1 and has to be included for column i
                    change = 1;
                    nseqi++;
                    for (int j = 1; j <= L; ++j)
                        n[j][(int) X[k][j]]++;
                } else if (X[k][i - 1] == GAP && X[k][i] !=
                                                 GAP) {  // ... if sequence k WAS included in i-1 and has to be thrown out for column i
                    change = 1;
                    nseqi--;
                    for (int j = 1; j <= L; ++j)
                        n[j][(int) X[k][j]]--;
                    ...
                    for (k = 0; k < N_in; ++k) {
                        if (in[k] && X[k][i] == GAP && X[k][j] < ANY) {
                            if (!n[j][(int) X[k][j]]) {
                                HH_LOG(ERROR) << "In " << __FILE__ << ":" << __LINE__ << ": " << __func__ << ":"
                                              << std::endl;
                                HH_LOG(ERROR) << "\tDi=" << i << ": n[" << j << "][X[" << k << "][" << j << "]]=0! (X["
                                              << k << "][" << j << "]=" << X[k][j] << ")" << std::endl;
                            }
                            wi[k] += 1.0 / float(n[j][(int) X[k][j]] * naa);
                        }
...
void Alignment::WriteWithoutInsertsToFile(const char *alnfile,
    ...
    for (int k = 0; k < N_in; ++k)
        if (!(k == kss_pred || k == kss_conf || k == kss_dssp || k == ksa_dssp)
            && (keep[k] ||
                display[k] == 2))  // print if either in profile (keep[k]>0) or display is obligatory (display[k]==2)
        {
            fprintf(alnf, ">%s\n", sname[k]);
            for (int i = 1; i <= L; ++i)
                fprintf(alnf, "%c", i2aa(X[k][i]));
...
void Alignment::WriteToFile(std::stringstream &out, const char format[]) {
    ...
    if (!format || !strcmp(format, "a3m")) {
        ...
        for (int k = 0; k < N_in; ++k)
            if (!(k == kss_pred || k == kss_conf || k == kss_dssp || k == ksa_dssp)
                && (keep[k] || display[k] ==
                               2))  // print if either in profile (keep[k]>0) or display is obligatory (display[k]==2)
                out << ">" << sname[k] << std::endl << seq[k] + 1 << std::endl;
