...
void ReadQueryFile(Parameters &param, FILE *file, char &input_format, char use_global_weights, HMM *q, Alignment *qali,
                   char infile[],
...
void PrepareQueryHMM(Parameters &par, char &input_format, HMM *q, cs::Pseudocounts<cs::AA> *pc_hhm_context_engine,
                     cs::Admix *pc_hhm_context_mode,
                     const float *pb, const float R[20][20]);

// Do precalculations for q and t to prepare comparison
void PrepareTemplateHMM(Parameters &par, HMM *q, HMM *t, int format, float linear_tranistion_probs, const float *pb,
                        const float R[20][20]);
